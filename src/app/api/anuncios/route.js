...
export async function POST(req) {
  try {
    ...
    const promises = nuevosAnuncios.map(({ id, titulo, activo }) => {
      if (!id) return Promise.resolve(); // Ignorar si no hay id
      const validTitulo = titulo || "";  // mantener string
      const validActivo = typeof activo === "boolean" ? activo : undefined;

      if (validActivo === undefined) {
        // Solo actualiza título si activo es indefinido
        return pool.execute("UPDATE anuncios SET titulo = ? WHERE id = ?", [
          validTitulo,
          id,
        ]);
      }
